...
public class HtmlCleaner {
	...
	public static String stripTags(String html) {
		// Removes all HTML tags using a regular expression
		// Remove DOCTYPE declaration
		html = html.replaceAll("(?i)<!DOCTYPE[^>]*>", "");
		// Remove XML declaration
		html = html.replaceAll("(?i)<\\?xml[^>]*\\?>", "");
		return html.replaceAll("<[a-zA-Z][^>]*>|</[a-zA-Z][^>]*>", "");
	}
	...
	public static String stripEntities(String html) {
		// Replaces all HTML entities with their Unicode character equivalent or removes unrecognized entities

		// First convert &dash; to hyphen
		html = html.replaceAll("&dash;", "");
	...
	public static String stripComments(String html) {
		// Removes all HTML comments using a regular expression
	...
	public static String stripElement(String html, String name) {
		// Escape the element name to handle special regex characters like [], (), etc.
		// This ensures the regex pattern treats element names literally
		String regex = String.format("(?is)<%s\\b[^>]*?>.*?</%s\\s*>", 
